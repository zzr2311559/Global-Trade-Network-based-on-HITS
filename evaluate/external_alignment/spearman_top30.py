...
def run_evaluation():
    ...
    for file in result_files:
        ...
        # ==========================================================
        # 提取 Top-K 国家
        # 设定 K 的值，比如只看排名前 30 的核心国家
        K = 30 
        
        # 按照 Hub Score 从大到小排序，并使用 .head(K) 截取前 K 行
        merged_df = merged_df.sort_values(by='Hub Score (出)', ascending=False).head(K)
        # ==========================================================
    ...
    print(f"\nSpearman Rank Correlation (Top-{K} Countries):")
...
def plot_trend(results_df):
    ...
    # 图表标题我也帮你加上了 Top-K 的标注
    plt.title(r'Impact of Geographic Penalty ($\gamma$) on Hub Ranking Alignment (Top-K)', fontsize=15, pad=15)
