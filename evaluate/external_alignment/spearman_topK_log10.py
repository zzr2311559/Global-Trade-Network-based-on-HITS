...
# 修改点 1：修改输出图片名称，标注这是 log10 的实验结果
OUTPUT_IMG_PATH = os.path.join(os.path.dirname(__file__), "spearman_topK_log10_trend.png")

def run_evaluation():
    # 修改点 2：匹配刚刚用 main_log10.py 跑出来的文件
    result_files = [f for f in os.listdir(RESULT_DIR) if f.endswith('.csv') and 'log10_geobiased' in f]
    
    if not result_files:
        print("未找到 log10_ 开头的 CSV 文件，请检查 result 目录或先运行 main_log10.py")
        return
    ...
    plt.title(r'Log10 Model Sensitivity: Impact of Top-K on LPI Correlation', fontsize=15, pad=15)
    ...
    print(f"\n Log10 敏感度图片已保存至: {OUTPUT_IMG_PATH}")
